...
router.post("/sendverification", async (req, res) => {
  ...
  function myhtml(name){
    ...
    <div style="width: 50%; margin: auto;text-align: center; background-color: antiquewhite;padding:  50px 20px  50px 20px; border-radius: 5px">
        ...
        <p>hey... to ensure account security we'll need to verify your identity.</p>
        <button style="background-color: burlywood; border-radius: 5px; border: none; padding:  5px 15px  5px 15px;">Verify</button>
    </div>
    `
  ...
  router.post("/sendcampain/:id", async (req, res) => {
    ...
    try {
      ...
      const subscriber_data= await Subscriber.find({user_id:req.params.id}).populate("user_id").lean().exec()
